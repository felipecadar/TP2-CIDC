...
if __name__ == "__main__":
    ...
    data = data.groupby('pid')['track_uri'].apply(list).reset_index(name='tracks')
    itemSetList = data['tracks'].tolist()
    
    if args.run_grid_search:
        results = grid_search(args)
        for result in results:
            print(f"Min support: {result['min_support']}, Min confidence: {result['min_confidence']}, Rule counts: {result['rule_counts']}")
